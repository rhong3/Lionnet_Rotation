...
def cbtest(tesample):
    ...
    for itr in range(len(tesample['ID'])):
        ...
        pred_cal = pred / 255
        tp = np.sum(pred_cal * y)
        predicted = np.sum(pred_cal)
        label = np.sum(y)
        ...
        metric.append([teid, ppv, F])

        io.imsave(output + '/' + teid + '_pred.tif', pred.astype(np.uint8))
        os.remove(output + '/' + teid + 'nuke.tif')
        os.remove(output + '/' + teid + 'gap.tif')

    metricpd = pd.DataFrame(metric, columns=['image', 'PPV', 'F_score'])
    metricpd.to_csv(output + '/' + 'metrics.csv', index=False)
