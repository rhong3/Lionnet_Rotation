...
for i in range(1, 24):
    if i < 10:
        ...
        ptt = str(i)
    try:
        image = io.imread("../5000C1-FB323A_CSC_Rd1 #{}.tif".format(ptt))
    ...
    for j in range(np.shape(image)[0]):
        try:
            os.mkdir('../stage_1_train/5000C1-FB323A_CSC_Rd1_{}_{}'.format(str(i), str(j+1)))
        except FileExistsError:
            pass
        try:
            os.mkdir('../stage_1_train/5000C1-FB323A_CSC_Rd1_{}_{}/images'.format(str(i), str(j+1)))
        except FileExistsError:
            pass
        try:
            os.mkdir('../stage_1_train/5000C1-FB323A_CSC_Rd1_{}_{}/label'.format(str(i), str(j+1)))
        except FileExistsError:
            pass
        ...
        io.imsave('../stage_1_train/5000C1-FB323A_CSC_Rd1_{}_{}/images/5000C1-FB323A_CSC_Rd1_{}_{}_pad.png'.
                  format(str(i), str(j+1), str(i), str(j+1)), img)
        io.imsave('../stage_1_train/5000C1-FB323A_CSC_Rd1_{}_{}/label/Combined_pad.png'.
                  format(str(i), str(j+1), str(i), str(j+1)), label[j, :, :])

        selem = mph.disk(2)
        erlabel = mph.erosion(label[j, :, :], selem)
        io.imsave('../stage_1_train/5000C1-FB323A_CSC_Rd1_{}_{}/label/ER_Combined_pad.png'.
                  format(str(i), str(j + 1), str(i), str(j + 1)), erlabel)

        selem = mph.disk(2)
        dllabel = mph.dilation(label[j, :, :], selem)
        io.imsave('../stage_1_train/5000C1-FB323A_CSC_Rd1_{}_{}/label/DL_Combined.png'.
                  format(str(i), str(j + 1), str(i), str(j + 1)), dllabel)

        gap = dllabel - erlabel
        io.imsave('../stage_1_train/5000C1-FB323A_CSC_Rd1_{}_{}/label/Gap_pad.png'.
                  format(str(i), str(j + 1), str(i), str(j + 1)), gap)

        new.append(['5000C1-FB323A_CSC_Rd1_{}_{}'.format(str(i), str(j + 1)), 1024, 1024, 0, 0, 0])
